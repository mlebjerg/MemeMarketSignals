...
request_url = "https://www.nyse.com/api/regulatory/threshold-securities/filter?selectedDate={}&market=&filterToken=XRT&max=10&offset=0&pageNumber=1&sortOrder=up&sortType="
symbol = "GME"
shapes_list = []
...
if not l300.empty:
    l300["tradeDate"] = pd.to_datetime(
        l300["tradeDate"], format="%Y-%m-%dT%H:%M:%S.%f%z")
...
if not l400.empty:
    l400["tradeDate"] = pd.to_datetime(
        l400["tradeDate"], format="%Y-%m-%dT%H:%M:%S.%f%z")
    fig.add_trace(go.Scatter(
...
if not l500.empty:
    l500["tradeDate"] = pd.to_datetime(
        l500["tradeDate"], format="%Y-%m-%dT%H:%M:%S.%f%z")
    fig.add_trace(go.Scatter(
...
if not l600.empty:
    l600["tradeDate"] = pd.to_datetime(
        l600["tradeDate"], format="%Y-%m-%dT%H:%M:%S.%f%z")
    fig.add_trace(go.Scatter(
...
if not l700.empty:
    l700["tradeDate"] = pd.to_datetime(
        l700["tradeDate"], format="%Y-%m-%dT%H:%M:%S.%f%z")
    fig.add_trace(go.Scatter(
...
if not l777.empty:
    l777["tradeDate"] = pd.to_datetime(
        l777["tradeDate"], format="%Y-%m-%dT%H:%M:%S.%f%z")
    fig.add_trace(go.Scatter(
...
if not l800.empty:
    l800["tradeDate"] = pd.to_datetime(
        l800["tradeDate"], format="%Y-%m-%dT%H:%M:%S.%f%z")
    fig.add_trace(go.Scatter(
...
if not l900.empty:
    l900["tradeDate"] = pd.to_datetime(
        l900["tradeDate"], format="%Y-%m-%dT%H:%M:%S.%f%z")
    fig.add_trace(go.Scatter(
...
if not l911.empty:
    l911["tradeDate"] = pd.to_datetime(
        l911["tradeDate"], format="%Y-%m-%dT%H:%M:%S.%f%z")
    fig.add_trace(go.Scatter(
...
l1000 = pd.DataFrame(Deal.select().where(
    Deal.volume == 1000).dicts().execute())
if not l1000.empty:
    l1000["tradeDate"] = pd.to_datetime(
        l1000["tradeDate"], format="%Y-%m-%dT%H:%M:%S.%f%z")
    fig.add_trace(go.Scatter(
...
l2100 = pd.DataFrame(Deal.select().where(
    Deal.volume == 2100).dicts().execute())
if not l2100.empty:
    l2100["tradeDate"] = pd.to_datetime(
        l2100["tradeDate"], format="%Y-%m-%dT%H:%M:%S.%f%z")
    fig.add_trace(go.Scatter(
...
fig.update_layout(title='{} share price'.format(
    symbol), template="plotly_dark", yaxis_title='Stock Price (USD)', shapes=shapes_list,)
fig.show()
